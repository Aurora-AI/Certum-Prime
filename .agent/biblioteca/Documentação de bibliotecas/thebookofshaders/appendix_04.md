...
When a browser (the *JavaScript **VM***) **executes** or **interprets** a piece of JS, it has no clue about which variable is what and which function does what (with the notable exception of **TypedArrays**).
Therefore it can't optimize anything *upfront*, so it takes some time to read your code, to **infer** (deduce from the usage) the types of your variables and methods
and when possible, it will convert *some* of your code into assembly code that will execute much faster.
...
The worst is that every browser optimizes JS its way and the process is *hidden* from you ; you are powerless.
...
That's cold but that's what a **shader** is: *a compiled program executed on the GPU*.
...
Instead of using the magic word **`var`**, you'll have to *explicitly specify the type of each variable* you use, then the compiler will only see objects and primitives it knows how to handle efficiently.
The downside when you can't use the **`var`** keyword and must *specify everything*, is that you'll have to know the type of all the variables and know them well.
...
If a variable is called `var uid = XXX;`, chances are that you'll store an integer value in there and a `var y = YYY;` *might* be a reference to a floating point value.
...
The solution to this is to use **type casting** ; it will *make the compiler believe* that *`i`* is of type `float` without actually changing the type of *`i`*.
...
Which is strictly equivalent to dressing up a ***cat*** in a ***giraffe* outfit** and will work as expected ( `r` will store the result of `i` x `f`).
...
The variable **types** are also their own **class constructor** ; in fact a `float` variable can be thought of as an *`instance`* of a *`Float`* class.
...
For those who don't know, **overloading** an operator or a function roughly means: *'changing the behaviour of said operator or function depending on the operands/arguments'*.
...
there is also a `struct` type. These are like *objects* without methods ;
...
Data structures are nice as such but we *might* need to iterate or perform conditional tests at some point.
