...
Now that we have a better idea of how this will work, letâs replace the `if` statement with [`step()`](../glossary/?search=step), and instead of using 10x10 letâs use normalized values between 0.0 and 1.0:
...
So far, weâve only drawn two borders (bottom-left) of our rectangle. Let's do the other two (top-right). Check out the following code:
...
We can also think of the above example as an altitude map, where darker implies taller. The gradient shows us something similar to the pattern made by a cone. Imagine yourself on the top of that cone. The horizontal distance to the edge of the cone is 0.5. This will be constant in all directions. By choosing where to âcutâ the cone you will get a bigger or smaller circular surface.

![](distance-field.jpg)

Basically we are using a re-interpretation of the space (based on the distance to the center) to make shapes. This technique is known as a âdistance fieldâ and is used in different ways from font outlines to 3D graphics.
...
Letâs take a look at the distance field formula on *line 19*. There we are calculating the distance to the position on `(.3,.3)` or `vec3(.3)` in all four quadrants (thatâs what [`abs()`](../glossary/?search=abs) is doing there).
