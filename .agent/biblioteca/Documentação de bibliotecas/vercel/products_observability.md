# https://vercel.com/products/observability

Understand production from the inside out.
==========================================

With real-time infrastructure and traffic insights, Vercel is the mission control for frontend applications.

[Deploy Now](/new)[Get a Demo](/contact/sales)

Visitors

634,200

Fri May 10 4pm - 5pm

90

90

App Logs

13:39

24:59

GET200

/app/front/apps/site/testing

24.52

GET200

/status/api

24.50

GET200

/docs

24.43

GET200

/api/jwt

Top Sources

Visitors

google.com

259,010

vercel.com

132,821

x.com

51,280

ycombinator.com

27,102

splitbee.com

20,186

linear.com

18,281

github.blog

13,021

Alerts

Pro

Enterprise

Proactive Anomaly Detection and Alerting.
-----------------------------------------

See traffic spikes and usage anomalies to pinpoint and resolve issues before they impact users.

![Graph showing a 5XX error rate anomaly on the '/api/auth' route. Vercel Agent is investigating the anomaly.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/alert-investigation-errors.svg)![Graph showing a 5XX error rate anomaly on the '/api/auth' route. Vercel Agent is investigating the anomaly.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/alert-investigation-errors-dark.svg)![Graph showing a 5XX error rate anomaly on the '/api/auth' route. Vercel Agent is investigating the anomaly.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/alert-investigation-investigating.svg)![Graph showing a 5XX error rate anomaly on the '/api/auth' route. Vercel Agent is investigating the anomaly.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/alert-investigation-investigating-dark.svg)

Query

Pro

Enterprise

Quick, accurate debugging.
--------------------------

Detect and diagnose issues in your web applications by surfacing errors, traffic, and performance data with custom queries and visualizations.

![An example of a monitoring chart. It shows the number of requests that responded with 429 over the past 2 weeks. In a table below, it shows how many 429s were returned for every host and path pair.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/monitor-track.svg)![An example of a monitoring chart. It shows the number of requests that responded with 429 over the past 2 weeks. In a table below, it shows how many 429s were returned for every host and path pair.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/monitor-track-dark.svg)

**Track and analyze the performance** of different parts of your site right in the Vercel dashboard. Leverage example queries to quickly resolve issues and optimize your projects.

![An example of the UI to configure a monitoring query. You can set the property to visualize, the number of rows to return, the filters and how to group the results.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/monitor-create-queries.svg)![An example of the UI to configure a monitoring query. You can set the property to visualize, the number of rows to return, the filters and how to group the results.](https://lishhsx6kmthaacj.public.blob.vercel-storage.com/monitor-create-queries-dark.svg)

**Create queries** to debug and optimize bandwidth, errors, performance, and bot traffic issues.

Logs

All plans

View, search, inspect.

Understand your application behavior with a view of all logs generated by every function in every deployment environment.

View all runtime logs generated by Vercel's infrastructure **across all environments.**

2M logs total found...

Time

Host

Request

Message

13:39:24.59

200acme.co

POST/api/create

Wrote 5 entries to database.13:39:24.57

200acme.co

GET/docs

User is authenticated13:39:24.50

200acme.co

GET/docs

13:39:24.51

500acme.co

POST/api/feature-\_

[FetchError]: Failed to load database13:39:24.50

200acme.co

GET/api/jwt

Expired. Generating new token...13:39:24.42

200acme.co

POST/stream/exter...

13:39:24.40

200acme.co

GET/

13:39:24.31

200acme.co

GET/changelog

Request Metrics

GET

/blog-posts

217ms

GET

/post/1

110ms

GET

/post/2

111ms

GET

/post/3

108ms

GET

/track-view

38ms

See the status, duration, URL, and a trace for **each outgoing request**.

Copy share URL

Export to CSV

Export to JSON

2M logs total found...

Time

Host

Request

Message

13:39:24.59

200acme.co

POST/api/create

Wrote 5 entries to database.13:39:24.57

200acme.co

GET/docs

User is authenticated13:39:24.50

200acme.co

GET/docs

13:39:24.51

500acme.co

POST/api/feature-\_

[FetchError]: Failed to load database13:39:24.50

200acme.co

GET/api/jwt

Expired. Generating new token...13:39:24.42

200acme.co

POST/stream/exter...

13:39:24.40

200acme.co

GET/

13:39:24.31

200acme.co

GET/changelog

**Inspect and share** your runtime logs without any third-party integrations.

Third Party

Pro

Enterprise

Instrument, collect, and send data with flexibility.
----------------------------------------------------

Datadog

New Relic

More...

0ms

166ms

333ms

500ms

**Use your favorite tools**. With OpenTelemetry (**OTEL**), send traces from your functions to application performance monitoring (**APM**) vendors.

[View Integrations](/integrations#analytics)

acme-site

4,815,162 Logs

**Log Drains**. Share data with your favorite logging provider for advanced alerting.

Break out of the black box.
---------------------------

[### Proactively defend.

Track anomalies to shut down threats as they happen with advanced monitoring.](/security)[### Prevent regressions.

Ship new work without breaking customer experiences, and know exactly where to iterate after.](/security)[### Total transparency.

Granular insights into application performance to ensure you're always up and running.](/security)

Web Analytics

All plans

Understand your engagement.
---------------------------

Providing comprehensive insights into your website’s visitors, tracking topic pages, referrers, and demographics like location, operating systems, and browser info.

[![](/vc-ap-vercel-marketing/_next/static/media/logo-vercel-light.b25fb3c2.svg?dpl=dpl_DAmfQ7ShxgPNUreS74eoRtAwLkga)![](/vc-ap-vercel-marketing/_next/static/media/logo-vercel-dark.b25fb3c2.svg?dpl=dpl_DAmfQ7ShxgPNUreS74eoRtAwLkga)

Deploy Now](/new)[Get a Demo](/contact/sales)

**First-party, privacy-friendly.** Data sent through your domain to avoid ad blockers.

**Find your top performers.** Identify your best sources and most effective content.

google.com

1,142,712 visitors

/login

714K

/dashboard

521K

/new

249K

news.ycombinator.com

728,101 visitors

reddit.com

589,102 visitors

Track **custom events** without third-party scripts, for **Pro and Enterprise**.

import { track } from '@vercel/analytics'

function SignupButton() {

return (

<button

onClick={() => {

track('Signup', {

location: 'Hero'

});

}}

>

Web Analytics Plus: **UTM parameters**, **more events**, and longer **report windows**.

Wed 10 Apr 2024

buy-it-now

40,171 clicks

Sun 10 Dec 2023

buy-it-now

21,291 clicks

Speed Insights

All plans

Real users, real performance data.

Performance from the right point of view. See detailed views of your website’s performance metrics with Speed Insights, facilitating informed decisions for optimization.

Real data points collected from your users’ devices for an **authentic evaluation** of their experiences.

Last 7 Days

P75

P90

P95

P99

First Contentful Paint

1.01

s

-30%

P75

100

90

50

25

Poor (5)

/faqs

21

/about

56

/pricing

32

To Improve (12)

/support

12

/holidays

38

/collaboration

24

Great (100+)

/home

89

/buy

31

/cart

12

Understand Core Web Vitals like **First Contentful Paint**, **Largest Contentful Paint**, **Cumulative Layout Shift**.

92

Real Experience Score

+25%

The combined score of your Web Vitals experienced by your visitors.

First Contentful Paint

1.01

s

Largest Contentful Paint

1.42

s

Cumulative Layout Shift

0.02

Interaction to Next Paint

1.01

s

[January 19th, 2024

**How Core Web Vitals affect SEO**. Understanding your application's Google page experience ranking and Lighthouse scores.

![Avatar for cramforce](/api/www/avatar?s=48&u=cramforce&dpl=dpl_DAmfQ7ShxgPNUreS74eoRtAwLkga "Avatar for cramforce")![Avatar for 3mdistal](/api/www/avatar?s=48&u=3mdistal&dpl=dpl_DAmfQ7ShxgPNUreS74eoRtAwLkga "Avatar for 3mdistal")

Malte Ubl, Alice Alexandra Moore

Read](/blog/how-core-web-vitals-affect-seo)

**Ready to deploy?** Start building with a free account. Speak to an expert for your *Pro* or Enterprise needs.

[Start Deploying](/new)[Talk to an Expert](/contact/sales)

**Explore Vercel Enterprise** with an interactive product tour, trial, or a personalized demo.

[Explore Enterprise](/try-enterprise)