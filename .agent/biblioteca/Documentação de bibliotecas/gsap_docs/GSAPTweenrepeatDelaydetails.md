...
Details
â
Gets or sets the amount of time in seconds between repeats. For example, if
repeat
is 2 and
repeatDelay
is 1, the tween will play initially, then wait for 1 second before it repeats, then play again, then wait 1 second again before doing its final repeat. You can set the initial
repeatDelay
value via the
vars
parameter, like:
gsap.to(obj, {duration: 1, x: 100, repeat: 2, repeatDelay: 1});
...
myTween.repeat(2).yoyo(true).repeatDelay(0.5).play();
