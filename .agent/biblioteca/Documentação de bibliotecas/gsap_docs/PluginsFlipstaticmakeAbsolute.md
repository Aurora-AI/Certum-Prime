...
Parameters
targets
: String | Element | Array | NodeList | FlipState
...
Sets all of the provided target elements to
position: absolute
while retaining their current positioning. This functionality is already built into the standard
Flip.from()
and
Flip.fit()
using the
absolute: true
special property, but this method is exposed in case you'd like to use it independently. You could literally make all the elements in the document position: absolute by running
Flip.makeAbsolute("\*")
