...
- [only applies to
type: "rotation"
] The current rotation (in degrees) of the Draggable instance. This is the rotation transform, as in the CSS
...
x
property by calling the Draggable's
update()
method. Basically that re-synchronizes it. Again, this is not necessary unless other code (outside Draggable) alters the target element's position.
Previous
.pointerY
Next
.startX
