...
vars
: Object
An object defining the value to which each property should be set. For example, to set element's
left
to 100 and element's
top
to 200, do this:
myTimeline.set(element, {left: 100, top: 200});
position
: [ Number | String ]
(default =
"+=0"
...
Adds a
gsap.set()
to the end of the timeline (or elsewhere using the "position" parameter) - this is a convenience method that accomplishes exactly the same thing as
add( gsap.set(...) )
...
â
By default, set() calls are added to the
end
